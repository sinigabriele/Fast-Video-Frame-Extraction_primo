...
package com.ducky.fastvideoframeextraction.ml
...
import com.ducky.fastvideoframeextraction.data.*
...
class MoveNet(private val interpreter: Interpreter, private var gpuDelegate: GpuDelegate?) :
    ...
    companion object {
        ...
        // TFLite file names.
        private const val LIGHTNING_FILENAME =   "movenet8unit.tflite"  //"movenet_lightning.tflite" // "4.tflite"//
        private const val THUNDER_FILENAME = "movenet8unit.tflite"  //"movenet_thunder.tflite"
    ...
    override fun estimatePoses(bitmap: Bitmap): List<Person> {
        var inferenceStartTimeNanos = SystemClock.elapsedRealtimeNanos()
        if (cropRegion == null) {
        ...
        cropRegion?.run {
            ...
            inferenceStartTimeNanos = SystemClock.elapsedRealtimeNanos()
...
//        lastInferenceTimeNanos =
//            SystemClock.elapsedRealtimeNanos() - inferenceStartTimeNanos
        Log.d("INFERENCE TIME","Inference time : " +(lastInferenceTimeNanos.toFloat()/1000000).toString() )
